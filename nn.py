...
class searchnet:
	...
	def setstrength(self, fromid, toid, layer, strength):
		if layer == 0:
			...
			table = 'hiddenurl'
		res = self.con.execute('select rowid from %s where fromid = %d and toid = \
			%d' %(table, fromid, toid)).fetchone()
		if res == None:
	...
	def setupnetwork(self, wordids, urlids):
		...
		self.wo = [[self.getstrength(hiddenid, urlid, 1) for 
			urlid in self.urlids] for hiddenid in hiddenids]
